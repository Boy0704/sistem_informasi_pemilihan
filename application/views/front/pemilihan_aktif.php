        ...
        <div class="page-content header-clear-medium">
            <div class="content">
                ...
                <h5 class="bold top-10 bottom-10 center-text">
                        <i class="fa fa-check-square color-green2-light fa-4x"></i></h5>
                <p class="under-heading center-text color-theme bottom-5 top-10"
                    style="line-height: 20px;padding: 0px 15px;">
                    Pemilihan aktif sudah bisa dilakukan pemilihan sesuai waktu yang ditetapkan Panitia.</p>
            </div>
           ...
           <div class="content top-15">
                <?php 
                if ($this->db->get_where('pemilihan', array('status'=>1))->num_rows() == NULL ) {
                    echo '<a href="#" class="button button-full button-m shadow-large button-round-small bg-highlight top-30 bottom-0">Belum Ada Pemilihan Aktif</a>';
                } else {
                foreach ($this->db->get_where('pemilihan', array('status'=>1))->result() as $key => $rw) {
